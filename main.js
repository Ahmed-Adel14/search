...
function handleDisplayProduct(data) {
  ...
  ${"★".repeat(product.rate)}${"☆".repeat(5 - product.rate)}
                        ...
                        <a href="./product-title.html?category=${product.category}">${product.title}</a>
                        <div class="para2">
...
searchbutton.addEventListener("click", () => {
    const productfilter = products.filter((product) => product.title.toLowerCase().includes(searchInput.value.toLowerCase()))
